...
- 연결 유지 비용(서버 리소스/스케일) : 연결이 계속 살아있어서 동시 접속자가 많아지면 서버 부담↑
    - 보완:
        - STOMP Broker 사용(또는 외부 브로커)
        - 서버 확장 시 Redis Pub/Sub, RabbitMQ 등으로 메시지 브로드캐스트 분산
        - 커넥션/세션 제한, Idle timeout 설정
- 인증/권한(HTTP 필터 체인과 분리) : WebSocket은 한 번 연결되면 이후 메시지 프레임엔 HTTP 인증이 자동 적용되지 않음
    - 보완:
        - HandshakeInterceptor 또는 ChannelInterceptor(CONNECT)에서 JWT 검증
        - room 권한 체크(방 참여자만 publish/subscribe)
