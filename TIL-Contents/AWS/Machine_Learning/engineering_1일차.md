...
- 열기반  : 압축이 좋다
    - Parquet
    - Optimaized Row Colummer(ORC)

- 객체 표기법
    - Json
    - jvascript Object Notation Lines (JSONL)
...
- 배치 데이터
    - 그룹 단위로 수집 및 처리
        - `일회성 작업` 또는 `반복적인` 스케줄
    - 전체 데이터 세트로 모델을 훈련할 수 있음

- 스트리밍 데이터 : `(수집 - 저장 - 처리 - 분석 - 저장)` 파이프라인을 통함
    - 지속적인 데이터 생성 및 처리
    - 준실시간 작업 또는 예측에 이상적
...
- 누락된 데이터 처리
    - 원인 파악 
        - 무작위 누락 ex) 키 - 몸무게 -> 비만 일때 사람들이 답변하는 몸무게에 대한 데이터가 정확하지 않음
        - 완전 무작위 누락 ex) 관련성이없거나 측정이안되는 변수가 존재 할때 
        - 비무작위 누락 ex) 소득수준에 대한 질문을 하는데 -> 소득이 높은사람이 답변을 거부한경우 -> 이런경우 이걸 해결하기 위해 -> 전문가 초빙 -> 전문가가 그 사람의 다른 정보로 추측을해야함

### 이상치 탐지 및 처리
> 이상치는 정상적인 동각과 다른 희귀 값이다.

![alt text](../../a_images/defrente_data.png)
...
### 범주형 인코딩
- 이진
- 순서형 : 순서는 있으나 간격이 동일할지는 모른다. ex) 학력, 성적
    - 정렬 카테로리
        - 숫자 값
- 명목형 : 순서가없음 ex) 남/여
    - 미정렬 카테고리
        - 레이블 인코딩 : 테이블에 존재하는 값을 숫자화 하여 표현
            - ![alt text](../../a_images/label_incode.png)
        - 원-핫 인코딩 : 쪼개는것 -> 크기가 늘어남으로 -> 차원의 저주(알고리즘 실행 저하 등)의 현상이 발생할 수 있음
            - ![alt text](../../a_images/one_hot_incode.png)

### 수치적 특성 추출
> 넓은 범위 또는 높은 편차가 있을 때 사용
...
-> 이러한 특성들은 선택적으로 제거하여 사용할 수 있다.

### 특성 분할
> 하나의 특성을 3개로 나누거나 할 수 있음

> 또는 특성을  하나로 줄일 수 도 있다.

ex) 주택가격을 분석하는 데이터에 대해서 불필요한건 줄이고 필요한건 세분화 할 수 있단는 것
